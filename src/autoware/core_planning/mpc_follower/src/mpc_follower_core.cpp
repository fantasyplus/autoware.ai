    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  pnh_.param("in_waypoints_name", in_waypoints, std::string("mpc_waypoints"));
  ...
  //insert by xt
  is_pub_follower.data=0x03;//gagagagag
  sub_pub_flag=nh_.subscribe("pub_flag",1,&MPCFollower::callbackFromPubFlag,this);

};

//insert by xt
void MPCFollower::callbackFromPubFlag(const std_msgs::Int8 msg){
...
void MPCFollower::timerCallback(const ros::TimerEvent &te)
{
  ...
  {
    ROS_ERROR("[MPC] MPC is not solved. ref_traj_.size() = %d, my_position_ok_ = %d,  my_velocity_ok_ = %d,  my_steering_ok_ = %d",
...
void MPCFollower::publishCtrlCmd(const double &vel_cmd, const double &acc_cmd, const double &steer_cmd)
{
  ...
  if(is_pub_follower.data==0x03){
      pub_steer_vel_ctrl_cmd_.publish(cmd);
      //ROS_INFO("mpc get flag %d,start navi mode",is_pub_follower.data);
  }
  // else{
  //     cmd.cmd.linear_velocity = 0;
  //     cmd.cmd.linear_acceleration = 0;
  //     pub_steer_vel_ctrl_cmd_.publish(cmd);
  //     ROS_INFO("mpc get flag %d,stop navi mode",is_pub_follower.data);
  // }
