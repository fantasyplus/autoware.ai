...
void PurePursuitNode::initForROS()
{
  ...
  private_nh_.param(
    "minimum_lookahead_distance", minimum_lookahead_distance_, 2.0);
  private_nh_.param("update_rate", update_rate_, 30.0);
  nh_.param("vehicle_info/wheel_base", wheel_base_, 1.1);

  // setup subscriber
  sub1_ = nh_.subscribe("final_waypoints_follow", 10,//gagagaga
  ...
  //insert by xt
  is_pub_follower.data=0x00;
  sub_pub_flag=nh_.subscribe("pub_flag",1,&PurePursuitNode::callbackFromPubFlag,this);

  pub_flag=nh_.advertise<std_msgs::Int8>("pub_flag",1);
}

//insert by xt
void PurePursuitNode::callbackFromPubFlag(const std_msgs::Int8 msg){
  ROS_INFO("get flag is :%d",msg.data);
  is_pub_follower=msg;
}
...
void PurePursuitNode::run()
{
  ...
  while (ros::ok())
  {
    ...
    {
      ROS_WARN("Necessary topics are not subscribed yet ... pos%d wp%d velo%d",is_pose_set_,is_waypoint_set_,is_velocity_set_);
      loop_rate.sleep();
      continue;
    }
    ...
    ROS_INFO("can_get_curvature is %d",can_get_curvature);
    ...
    //当waypoints为0时会out_of_range，直接注释掉
  ...
  const bool& can_get_curvature, const double& kappa) const
{
 ...
 if(is_pub_follower.data==0x04){
    pub2_.publish(ccs);
    ROS_INFO("pub ctrl_raw,velo=%f,angle    =%f",ccs.cmd.linear_velocity,ccs.cmd.steering_angle / M_PI * 180.0);
 }
//  else{
//    ccs.cmd.linear_velocity = 0;
//   ccs.cmd.linear_acceleration = 0;
//   pub2_.publish(ccs);
//       ROS_INFO("pure_pursuit get flag %d,stop navi mode",is_pub_follower.data);
//   }
}
...
{
  if(msg->waypoints.empty() && is_pub_follower.data==0x04){
    ROS_WARN("----------------get empty waypoint-----------------");
    ...
    // std_msgs::Int8 stop_command;
    // stop_command.data=0x00;//一旦发了这个,nmea_follower会直接不发路径
    // pub_flag.publish(stop_command);
    ROS_ERROR("===============stop the car================");
    return ;
  }
  ...
  else
  {
    ...
    //ROS_INFO("msg->waypoints.size():%d",msg->waypoints.size());
  }
