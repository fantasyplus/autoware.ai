...
AstarNavi::AstarNavi() : nh_(), private_nh_("~")
{
  ...
  current_pose_sub_ = nh_.subscribe("initialpose", 1, &AstarNavi::currentPoseCallback, this);
...
void AstarNavi::currentPoseCallback(const geometry_msgs::PoseWithCovarianceStamped &msg)
{
  ...
  current_pose_local_.pose.pose = transformPose(
      current_pose_global_.pose.pose, getTransform(costmap_.header.frame_id, current_pose_global_.header.frame_id));
  ...
  ROS_INFO_STREAM("Subscribed start pose and transform from " << msg.header.frame_id << " to "
                                                             << current_pose_local_.header.frame_id << "\n"
                                                             << current_pose_local_.pose.pose);
}
...
void AstarNavi::run()
{
  ...
  while (ros::ok())
  {
    ...
    bool result = astar_.makePlan(current_pose_local_.pose.pose, goal_pose_local_.pose);
...
void AstarNavi::publishWaypoints(const nav_msgs::Path &path, const double &velocity)
{
  ...
  for (const auto &pose : path.poses)
  {
    ...
    wp.pose.pose.position.z = current_pose_global_.pose.pose.position.z; // height = const
...
void AstarNavi::publishStopWaypoints()
{
  ...
  pose.pose = current_pose_global_.pose.pose;
