...
void AstarSearch::poseToIndex(const geometry_msgs::Pose &pose, int *index_x, int *index_y, int *index_theta)
{
  ...
  *index_theta %= theta_size_;//在48个theta_size_中属于第几个，给search()用
}
...
bool AstarSearch::search()
{
  ...
  while (!openlist_.empty())
  {
    ...
    for (const auto &state : state_update_table_[top_sn.index_theta])
    {
      ...
      double move_cost = state.step;//gc 每一步的小弧长
      ...
      {
        //如果更新后的gc值比原来的gc值要小
        //若是OPEN状态，就更新更小的gc值(扩张)
        //若是CLOSE状态，就变为OPEN，更新更小的gc值(更新)
