...
void PointPillarsROS::pubDetectedObject(const std::vector<float>& detections, const std_msgs::Header& in_header)
{
  ...
  int num_objects = detections.size() / OUTPUT_NUM_BOX_FEATURE_;//每个bbox有7个特征，除一下就是有多少个bbox
...
{
  tf::Vector3 v = baselink2lidar.getOrigin();//getOrigin()得到baselink相对于lidar的坐标
  offset_z_from_trained_data_ = v.getZ() - TRAINED_SENSOR_HEIGHT_;//训练用的雷达高度和实际雷达高度的误差

  tf::Quaternion q = baselink2lidar_.getRotation();//baselink转换到lidar的四元数
  angle_transform_ = tf::Transform(q);//baselink转换到lidar的矩阵（只有角度）
...
void PointPillarsROS::pointsCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
  ...
  std::vector<float> out_detection;//网络输出的bounding box，纯数组形式
