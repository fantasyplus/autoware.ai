...
static void gnss_callback(const geometry_msgs::PoseStamped::ConstPtr& input)
{
  ...
  static pose previous_gnss_pose = current_gnss_pose;//初始化，只赋值一次
  ros::Time current_gnss_time = input->header.stamp;
  static ros::Time previous_gnss_time = current_gnss_time;//初始化，只赋值一次

  if ((_use_gnss == 1 && init_pos_set == 0) || fitness_score >= _gnss_reinit_fitness)//_gnss_reinit_fitness=500.0
  {
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    pthread_mutex_lock(&mutex);//防止和线程中的mapcallback冲突
    ...
    if (_use_imu == true && _use_odom == true)
      ...
      predict_pose_for_ndt = predict_pose;//only gnss
    ...
    Eigen::Matrix4f init_guess = (init_translation * init_rotation_z * init_rotation_y * init_rotation_x) * tf_btol;//平移乘旋转乘bask_link to lidar
    ...
    t2 = t * tf_btol.inverse();//map2lidar * lidar2base_link=map2base_link
    ...
    tf2::Matrix3x3 mat_l;  // localizer(lidar)
    ...
    */
    ...
    exe_time = std::chrono::duration_cast<std::chrono::microseconds>(matching_end - matching_start).count() / 1000.0;//us/1000=ms
...
int main(int argc, char** argv)
{
  ...
  //  ros::Subscriber map_sub = nh.subscribe("points_map", 1, map_callback);//在线程里订阅了
  ...
  pthread_t thread;
  pthread_create(&thread, NULL, thread_func, NULL);//map_callback
