...
void publish_pcd(sensor_msgs::PointCloud2 pcd, const int* errp = NULL)
{
  if (pcd.width != 0)
  {
    ...
    // pcl::fromROSMsg(pcd, *pcl_pc_ptr);
    // pcl::io::savePCDFileASCII("/home/ros/pcl_exe/icp_test/icp_demo/map/map.pcd", *pcl_pc_ptr);
    // printf("save map.pcd successfully\n");
    if (errp == NULL || *errp == 0)
    {
      stat_msg.data = true;
      stat_pub.publish(stat_msg);
      ROS_INFO("Pub Point Map Successfully!");
    }
  }
}
...
int main(int argc, char** argv)
{
  ...
  for (const std::string& pcd_path : pcd_paths)
  {
    ...
    if (boost::filesystem::is_regular_file(path))
    {
      ...
      ROS_INFO("get pcd file\n");
    }
    else if (boost::filesystem::is_directory(path))
    {
      // If directory
      ROS_INFO("get into directory\n");
